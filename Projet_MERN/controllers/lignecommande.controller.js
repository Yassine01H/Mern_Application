...
export const getLigneCommandes = async (req, res) => {
 try {
 const lc = await
LigneCommande.find().populate('refvoiture').populate('refcommande');
...
export const createLigneCommande = async (req, res) => {
 const { quantitecommandee, totligne,refvoiture,refcommande } = req.body;

 const newLigneCommande = new
LigneCommande({ quantitecommandee:quantitecommandee,
totligne:totligne,refvoiture:refvoiture,refcommande:refcommande })
try {
   ...
   de LC avec un id: ${id}`);
    ...
    refvoiture:refvoiture,refcommande:refcommande,_id: id };
   ...
   req.params.refcommande}).populate('refcommande').populate('refvoiture').exec()
   ;
